...
3. Read the ticket, its embedded acceptance criteria (Pattern A), any PM comment, and — if this is a bounce — Aled's note. **New skill in `claude-ops`:** if the ticket creates a new skill, the canonical file is `.claude/skills/<name>/SKILL.md` (auto-loaded at startup) — `public/downloads/<name>.md` is the web mirror only; create both, and add a row to the `CLAUDE.md` skills table.
