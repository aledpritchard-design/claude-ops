...
4. Run Claude Code on this ticket's branch. Before opening a PR: check whether an open PR for this ticket's branch already exists in the repo (use `list_pull_requests` filtered to the branch name). If an open PR exists on **the same branch** — do not open a second one; push new commits to that branch and continue with the handoff using the existing PR. If an open PR exists on **a different branch** for the same ticket (two runs used different branch conventions), post a comment noting the conflict, do not open a second PR, and take the Blocked path. Only open a new PR when none exists. Never merge.
