...
description: "Qualify Pipeline tickets from Backlog to Ready on behalf of the PM role. Use whenever asked to qualify, validate, or promote Pipeline backlog tickets, or when running the pipeline qualify pass. Scans all Pipeline funnels for Backlog issues and applies one of three outcomes — promote if complete, fix and promote if fixable, or flag to Aled if information is missing. Never touches Todo or beyond. Runs as a Cowork scheduled task, Linear connector only, no repo, no code."
