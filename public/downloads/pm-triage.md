...
- **Batch sequencing — pre-sequence shared-file cc-exec tickets before promoting to Todo.** After assessing all tickets individually in the run, and before writing any Todo state transitions, scan the full set of tickets identified for `cc-exec` + Todo promotion for pairs whose titles unambiguously target the same skill or file — e.g. two tickets both referencing "pm-triage", "exec skill", or the same filename in their titles. For each such pair, apply a priority-ordered `blockedBy` chain: the lower-priority ticket is blocked by the higher-priority one. Three rules:
  1. **Conservative only.** Apply only when the shared-file overlap is unambiguous from titles alone. Where the overlap is uncertain or titles are ambiguous, do not block — note the potential conflict for Aled in the routing comment instead.
  2. **Priority-ordered.** Higher-priority ticket is the blocker; lower-priority ticket is blocked. If priorities are equal, do not add a block — note for Aled.
  3. **Backstop preserved.** Exec's runtime `blockedBy` fail-safe is not removed or weakened by this step; it remains the backstop for conflicts not caught at routing time.
