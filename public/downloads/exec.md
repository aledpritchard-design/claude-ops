...
3. Read the ticket, its embedded acceptance criteria (Pattern A), any PM comment, and — if this is a bounce — Aled's note. When a criterion references a Sanity field (`siteSettings`, a schema type, or a GROQ query), read the existing Sanity schema files in the repo to determine whether the field is already defined. If it is not, create it in the schema and seed a default value as part of the implementation — or take the Blocked path if the schema change falls outside this ticket's scope. Never substitute a hardcoded constant for a criterion that explicitly requires a data-layer read.
