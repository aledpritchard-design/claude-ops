...
4. Run Claude Code on this ticket's branch. **Before opening a PR:** determine the repo's required CI checks by reading its `.github/workflows/` file, then run them locally in this order — (a) run the formatter to auto-fix format issues, (b) lint, (c) format-check, (d) type-check, (e) build (not every repo has all five — run exactly the ones CI enforces, not a guess). Only open the PR when all required checks pass. If a check fails and cannot be made green, do not open a partial PR — take the Blocked path instead. Never merge.
